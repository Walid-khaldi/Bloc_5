...
# Filtrer les conducteurs en retard
late_drivers = data["delay_at_checkout_in_minutes"] > 0
df_late_drivers = data[late_drivers]
...
fig, ax = plt.subplots()
sns.histplot(data=df_late_drivers, x="delay_at_checkout_in_minutes", hue="checkin_type", element="poly", bins=np.arange(0, 60*12, step=15), ax=ax) # (60*12) pour 12 heures

# Ajouter des titres et des étiquettes en français
ax.set_title("Répartition des retards des conducteurs")
ax.set_xlabel("Retard au check-out (en minutes)")
ax.set_ylabel("Nombre de conducteurs")
